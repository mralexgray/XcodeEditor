...
{
    XCGroup* localgroup = [XCGroup groupWithProject:project key:@"abcd1234" alias:@"Main" path:@"Source/Main" children:nil];

    assertThat(localgroup, notNilValue());
    assertThat([localgroup key], equalTo(@"abcd1234"));
    assertThat([localgroup alias], equalTo(@"Main"));
    assertThat([localgroup pathRelativeToParent], equalTo(@"Source/Main"));
    assertThat([localgroup members], empty());
}
...
- (void)test_allows_deleting_a_group_optionally_removing_also_the_contents
{
    ...
    XCGroup* localgroup = [project groupWithPathFromRoot:@"Source/Main/UserInterface/Components"];

    NSArray* groups = [project groups];
    NSLog(@"Groups now: %@", groups);

    [localgroup removeFromParentDeletingChildren:YES];
    [project save];
